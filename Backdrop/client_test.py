...
def test_detection(image_path):
    ...
    try:
        ...
        if result['success']:
            ...
            # Show ALL detections (even non-priority)
            if result['all_detections']:
                ...
                for i, det in enumerate(result['all_detections'], 1):
                    priority_marker = ""
                    for alert in result['priority_alerts']:
                        if alert['class'] == det['class']:
                            priority_marker = f" ⭐ [Priority {alert['priority']}]"
                            break
                    print(f"{i}. {det['class'].upper()} - {det['confidence']*100:.1f}%{priority_marker}")
                print()
            
            # Show priority alerts
            if result['priority_alerts']:
                print("🔔 PRIORITY ALERTS (Should have audio):")
                print("-" * 60)
                for alert in result['priority_alerts']:
                    print(f"🚨 {alert['message']}")
                    print(f"   Object: {alert['class']}")
...
if __name__ == "__main__":
    test_detection("test_image.jpg")  # or your image filename
